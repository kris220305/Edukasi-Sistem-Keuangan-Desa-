...
export function generateBKU(state: AppState): BKUEntry[] {
  ...
  // 3. Mutasi Kas (Penyetoran ke Bank / Pengambilan dari Bank)
  state.mutasiKas.forEach(m => {
    allTx.push({
      tanggal: m.tanggal, sortDate: m.tanggal,
      entries: [{
        tanggal: m.tanggal,
        kodeRekening: m.jenis === 'setor' ? '1.1.1.02' : '1.1.1.01',
        uraian: m.uraian,
        // Mencatat di kedua sisi agar menambah total tapi tidak merubah saldo (netto 0)
        penerimaan: m.jumlah,
        pengeluaran: m.jumlah,
        noBukti: m.noBukti,
        netto: 0,
      }]
    });
